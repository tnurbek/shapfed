...
from data_loader import get_custom_cifar10_train_loader, get_custom_cifar10_test_loader
...
test_data_loader =  get_custom_cifar10_test_loader(batch_size,random_seed, model_num)
train_data_loader = get_custom_cifar10_train_loader(batch_size,random_seed, shuffle, num_workers, save_name, model_num, intersection) 
...
class Client:
    def __init__(self, dataloader, client_id):
        self.model = models.resnet18(num_classes=10).to('cuda')
    ...
    def aggregate(self, coefficients):
        ...
        for client_id, client in enumerate(self.clients): 
            ...
            for name, param in client_weights.items():
                ...
                total_weights[name] += int(coefficients[client_id]) * param.to(total_weights[name].dtype)
